...
#-----------------------------------------------------------------------------------------
# Script information
script_name='FP ENVIRONMENT - SYSTEM LIBRARIES GENERIC - OPENJPEG'
script_version="1.0.0"
script_date='2022/05/18'

# Define library name, archive and repository
library_clean=true
library_name='openjpeg'
library_archive_generic='openjpeg.tar.gz'
library_archive_reference='v2.4.0.tar.gz'
library_archive_address='https://github.com/uclouvain/openjpeg/archive/refs/tags/'
...
generic_file_env='fp_system_libs_generic_openjpeg'
...
library_deps_zlib_include=${generic_path_building_destination}/zlib/include/
...
if [ ! -d "${library_path_bin}" -a ! -d "${library_path_lib}" ]; then
	...
	library_cmd_archive_configure=${library_cmd_archive_configure/'%LIBRARY_DEPS_ZLIB_MAIN'/$library_deps_zlib_main}
