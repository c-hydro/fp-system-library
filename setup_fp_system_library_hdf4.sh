...
generic_file_env='fp_system_libs_generic_hdf4'
...
if [ ! -d "${library_path_bin}" -a ! -d "${library_path_lib}" ]; then
	...
	library_cmd_archive_configure=${library_cmd_archive_configure/'%LIBRARY_DEPS_JPEG'/$library_deps_jpeg}
	
	if [ ! -d "${library_deps_zlib}" ]; then
		echo " ===> LIBRARY ZLIB NOT FOUND. EXIT"
		exit
	fi
	if [ ! -d "${library_deps_jpeg}" ]; then
		echo " ===> LIBRARY JPEG NOT FOUND. EXIT"
		exit
	fi
