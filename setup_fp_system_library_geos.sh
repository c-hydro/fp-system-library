...
#-----------------------------------------------------------------------------------------
# Script information
script_name='FP ENVIRONMENT - SYSTEM LIBRARIES GENERIC - GEOS'
...
library_clean=true
library_name='geos'
library_archive_generic='geos.tar.gz'
library_archive_reference='geos-3.8.0.tar.bz2'
library_archive_address='http://download.osgeo.org/geos/'
...
generic_file_env='fp_system_libs_generic_geos'
...
library_cmd_archive_unzip="tar -xjf %LIBRARY_ARCHIVE_BUILDING_SOURCE -C %LIBRARY_PATH_BUILDING_SOURCE --strip-components=1"
...
if [ ! -d "${library_path_bin}" -a ! -d "${library_path_lib}" ]; then
	...
	export GEOS_DIR=/home/fabio/Documents/Work_Area/Code_Development/Library/geos-3.9.1/lib

	# Export BINARY PATH(S)
	echo "PATH=${library_path_bin}:"'$PATH'"" >> $generic_file_env
	echo "export PATH" >> $generic_file_env
	
    # Export GEOS DIR PATH(S)
	echo "GEOS_DIR=${library_path_lib}" >> $generic_file_env
	echo "export GEOS_DIR" >> $generic_file_env
