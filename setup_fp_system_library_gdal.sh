...
#-----------------------------------------------------------------------------------------
# Script information
script_name='FP ENVIRONMENT - SYSTEM LIBRARIES GENERIC - GDAL'
...
library_clean=true
library_name='gdal'
library_archive_generic_group=('gdal.tar.gz')
library_archive_reference_group=('gdal-3.2.1.tar.gz')
library_archive_address_group=('http://download.osgeo.org/gdal/3.2.1/')
...
generic_file_env='fp_system_libs_generic_gdal'
...
library_deps_hdf4_main=${generic_path_building_destination}/hdf4
library_deps_openjpeg_main=${generic_path_building_destination}/openjpeg
library_deps_geos_main=${generic_path_building_destination}/geos
library_deps_geos_config=${generic_path_building_destination}/geos/bin/geos-config
library_deps_proj_main=${generic_path_building_destination}/proj
...
library_path_data=${library_path_building_destination}/share/gdal/
# ----------------------------------------------------------------------------------------
...
if [ ! -d "${library_path_bin}" -a ! -d "${library_path_lib}" ]; then
	...
	library_cmd_archive_configure=${library_cmd_archive_configure/'%LIBRARY_DEPS_GEOS_MAIN'/$library_deps_geos_main}
	library_cmd_archive_configure=${library_cmd_archive_configure/'%LIBRARY_DEPS_PROJ_MAIN'/$library_deps_proj_main}
	library_cmd_archive_configure=${library_cmd_archive_configure/'%LIBRARY_DEPS_OPENJPEG_MAIN'/$library_deps_openjpeg_main}
	library_cmd_archive_configure=${library_cmd_archive_configure/'%LIBRARY_DEPS_HDF4_MAIN'/$library_deps_hdf4_main}
	...
	if [ ! -d "${library_deps_geos_main}" ]; then
		echo " ===> LIBRARY GEOS NOT FOUND. EXIT"
		exit
	fi
	...
	if [ ! -d "${library_deps_openjpeg_main}" ]; then
		echo " ===> LIBRARY OPENJPEG NOT FOUND. EXIT"
		exit
	fi
	if [ ! -d "${library_deps_hdf4_main}" ]; then
		echo " ===> LIBRARY HDF4 NOT FOUND. EXIT"
		exit
	fi
	...
	# Export GDAL DATA PATH(S)
	echo "GDAL_DATA=${library_path_data}" >> $generic_file_env
	echo "export GDAL_DATA" >> $generic_file_env
